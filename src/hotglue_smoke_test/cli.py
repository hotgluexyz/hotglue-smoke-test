...
def _run_record_vcr(
    ...
    print(f"command [PYTHONPATH={env['PYTHONPATH']} python {record_vcr} {case}]")
...
def _run_playback_test(
    ...
    _print_section(f"Running Test: {tap_name} / {testcase}")
    _print_status("INFO", f"Starting test at: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")

    _run_record_vcr(tap_source_dir, tap_test_dir, testcase, test_suite, python_exe)
...
def cmd_run(args: argparse.Namespace) -> int:
    ...
    _print_section("Starting Test Execution")
    if args.case_name == "*":
    ...
    for testcase in cases:
        try:
            ...
            _print_status("SUCCESS", f"Test completed successfully: {testcase}")
        except (subprocess.CalledProcessError, OSError) as exc:
            failed.append(testcase)
            _print_status("ERROR", f"Test failed for {testcase}: {exc}")

    _print_section("Test Summary")
    _print_status("INFO", f"Total Tests: {len(cases)}")
    _print_status("INFO", f"Passed Tests: {len(passed)}")
    _print_status("INFO", f"Failed Tests: {len(failed)}")
    ...
    if failed:
        return 1
    _print_status("SUCCESS", "All tests completed successfully!")
    return 0
...
def build_parser() -> argparse.ArgumentParser:
    ...
    run_parser = subparsers.add_parser("run", help="Run colocated smoke tests")
    run_parser.add_argument("tap_name", help="Connector name without tap-/target- prefix")
    run_parser.add_argument("case_name", help="Test case name ending in _test, or * for all")
    run_parser.add_argument("--tap-directory", required=True, help="Path to connector repo root")
    run_parser.add_argument("--target", action="store_true", help="Run as target")
