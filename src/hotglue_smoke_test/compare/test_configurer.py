...
class TestConfigurer:
    ...
    def get_test_config(root_dir):
        ...
        if "ignore_files" in _test_config:
            test_config["ignore_files"] = list(
                set(test_config["ignore_files"] + _test_config["ignore_files"])
            )
        ...
        if "date_conversion_config" in _test_config:
            test_config["date_conversion_config"].update(
                _test_config["date_conversion_config"]
            )
